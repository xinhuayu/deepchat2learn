...
> **Critical release note:** Desktop voice conversation is working in the frozen baseline, but continuous voice conversation through mobile browsers is still not working reliably. This is a critical unresolved issue and a release blocker for mobile voice support; `v0.1.0` makes no mobile-voice readiness claim. The package is frozen while this is investigated, so no application behavior is changed here. Use desktop voice or typed interaction as the current workaround. Future work must reproduce the failure across target devices and browsers, then validate permissions, autoplay, WebRTC/Realtime transport, browser speech fallback, and network recovery before changing the frozen contract.
...
- Practice-session topic discovery and scope refinement: the first three rounds establish the learner’s definition and aim, scope and boundaries, then a central claim, hypothesis, mechanism, setting, or example; the first three exchanges are sent with the explicit `within the topic of ...` constraint to create a targeted digest and gist. The learner receives a short scope-confirmation prompt, and the next response can confirm or correct the focus while later requests remain inside the refined scope.
...
- Per-source digest preview; with `OPENAI_API_KEY`, direct digests are model-generated from a bounded per-source representation of up to 88,000 characters, with evidence-validated key points, a 12,000-token configurable structured-response allowance, and a safe extractive fallback. Explicit consolidation can batch bounded source chunks while the complete extracted material remains local.
...
- The academic-conversation skill establishes definition, scope, research aim, claim or hypothesis, setting, design, measures, evidence, and interpretation in sequence; later questions may open up only when they remain connected to the active topic, digest, or source evidence.
- Source conversation rounds use a compact academic dialogue protocol that frames definition, scope, research aim, claim or hypothesis, setting, design, measures, evidence, and interpretation before opening into related extensions; full research digestion and epidemiology critique guidance are reserved for source processing or explicit review requests so voice turns remain responsive.
...
- Desktop voice is the verified reference path for this frozen baseline. Mobile voice remains capability-driven rather than tied to a particular phone, operating system, or browser, but continuous mobile-browser voice conversation is a critical unresolved issue. Future mobile work must validate the Realtime/WebRTC path, browser speech fallback, permissions, autoplay, and recovery on target devices before the package can claim mobile voice readiness.
- Both practice and source conversations use only the compact academic-conversation guide for each dialogue turn. Full academic-research and epi-research guidance is reserved for source digestion or explicit review requests. Practice begins with a three-round framing phase; after the third completed round, prompts carry the targeted topic digest/gist, the topic, and up to five related exchanges. Source rounds progress from definition and scope through aim, claim or hypothesis, setting, design, measures, evidence, interpretation, and related extensions, using only the prepared digest and bounded evidence context. After a source is digested, source-answer prompts send only the topic, prepared digest/gist, compact exact-evidence options, and the three latest exchanges; generated source-question prompts use the prepared digest and recent history. The full document and raw chunks remain server-local for validation and fallbacks. A complete answer advances to a different related question; say "new question," "ask something new," or "another issue" to move on immediately, or use an explicit ending phrase to finish the session. Source-review skills remain focused on digesting and reviewing supplied materials.
