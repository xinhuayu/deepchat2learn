...
- Practice-session topic discovery and scope refinement: the first three rounds establish the learner’s definition and aim, scope and boundaries, then a central claim, hypothesis, mechanism, setting, or example; the first three exchanges are sent with the explicit `within the topic of ...` constraint to create a targeted digest and gist. The learner receives a short scope-confirmation question before later rounds use that digest.
