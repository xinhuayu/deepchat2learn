...
test('buildConsolidatedDigest accepts a concise interpretation when its separate evidence is exact', async () => {
  const source = {
    id: 'source-1',
    name: 'paper.txt',
    text: 'Retrieval practice boosts long-term retention in this course.',
  ...
  const digest = await buildConsolidatedDigest({
    ...
    coach: {
      async buildConsolidatedDigest() {
        return {
          mainArgument: 'The course evidence supports retrieval practice.',
          keyPoints: [{
            text: 'Retrieval practice is associated with stronger long-term retention.',
            evidence: 'Retrieval practice boosts long-term retention in this course.',
            sourceIds: ['source-1'],
            chunkIds: [chunks[0].id]
          }],
          importantTerms: [],
          evidence: [{ claim: 'Retrieval practice boosts long-term retention in this course.', chunkIds: [chunks[0].id] }],
          conflicts: [],
          openQuestions: [],
          warnings: []
        };
  ...
  assert.equal(digest.keyPoints[0].evidence, 'Retrieval practice boosts long-term retention in this course.');
});
