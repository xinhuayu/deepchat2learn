...
import { getAudioModel, getInteractionLimits, getRequestConfig, getRetentionConfig, getSessionBudgetConfig, getSourceLimits, getVoiceConfig, loadDotEnv, maxQuestionsForSourceMode, shouldLoadDotEnv } from '../src/config.mjs';
...
test('interaction limits reserve a 10 percent margin without changing voice silence timing', () => {
  assert.deepEqual(getInteractionLimits({}), {
    maxAnswerCharacters: 13_200,
    maxQuestionCharacters: 2_200
  });
  assert.deepEqual(getInteractionLimits({
    MAX_ANSWER_CHARACTERS: '15000',
    MAX_QUESTION_CHARACTERS: '2400'
  }), {
    maxAnswerCharacters: 15_000,
    maxQuestionCharacters: 2_400
  });
  const voice = getVoiceConfig({});
  assert.equal(voice.autoSubmitDelayMs, 5_000);
  assert.equal(voice.realtimeSilenceMs, 5_000);
});
...
test('voice config reads timing and retry overrides', () => {
  assert.deepEqual(getVoiceConfig({
    ...
    OPENAI_SOURCE_DIGEST_MAX_OUTPUT_TOKENS: '13200',
    OPENAI_SOURCE_CONVERSATION_MAX_OUTPUT_TOKENS: '4400',
    ...
    sourceDigestTimeoutMs: 180_000,
    sourceDigestMaxOutputTokens: 13_200,
    sourceConversationMaxOutputTokens: 4_400,
