...
test('final answer completion does not re-persist the finished session', async () => {
  ...
  const nonFinalBranch = submission.match(/else \{[^}]*\}/)?.[0];
  assert.ok(nonFinalBranch, 'non-final answer branch should be present');
  assert.match(nonFinalBranch, /persistClientSession\(\)/);
});
...
test('voice readiness messaging distinguishes browser audio from microphone permission', async () => {
  ...
  assert.match(app, /permission will be requested when you start voice/);
