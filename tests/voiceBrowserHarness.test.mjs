...
test('spoken question automatically starts answer listening when it finishes', async () => {
  const harness = await createHarness();
  const recognition = harness.recognition();
  harness.document.querySelector('#questionText').textContent = 'What is the paper\'s main argument?';

  harness.document.querySelector('#speakQuestion').click();
  await harness.flush();
  assert.equal(recognition.started, false);
