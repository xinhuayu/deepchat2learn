...
test('model gateway does not retry rejected schemas, incomplete output, or model refusals', async () => {
  for (const code of ['MODEL_REQUEST_INVALID', 'MODEL_OUTPUT_INCOMPLETE', 'MODEL_REFUSAL']) {
    let attempts = 0;
    const gateway = createModelGateway({
      textCoach: {
        async generalAnswer() {
          attempts += 1;
          throw new HttpError(502, `Provider failure: ${code}`, code);
        }
      },
      config: { maxTransientRetries: 3, timeoutMs: 100 }
    });
    ...
    await assert.rejects(
      () => gateway.runTextTask({ task: 'general_answer', input: { question: 'Should this retry?' } }),
      error => error.code === 'MODEL_GATEWAY_VALIDATION'
        && error.details?.retryable === false
        && error.details?.causeCode === code
    );
    assert.equal(attempts, 1, `${code} should not be retried`);
  }
});
...
test('model gateway falls back locally when an interactive request reaches its deadline', async () => {
  let fallbackCalls = 0;
  const gateway = createModelGateway({
    textCoach: {
      async generalAnswer(_question, { signal } = {}) {
        await new Promise(resolve => signal?.addEventListener('abort', resolve, { once: true }));
        return { mode: 'general', answer: 'Late remote answer', sourceGroundedClaims: [], additionalContext: [], unsupportedOrUnresolved: [], confidence: 'low' };
      }
    },
    config: {
      timeoutMs: 10,
      ...
      fallbackTextCoach: {
        async generalAnswer(question) {
          fallbackCalls += 1;
          return { mode: 'general', answer: `Local fallback for ${question}`, sourceGroundedClaims: [], additionalContext: [], unsupportedOrUnresolved: [], confidence: 'medium' };
        }
      }
    }
  });

  const result = await gateway.runTextTask({ task: 'general_answer', input: { question: 'Keep going?' } });

  assert.equal(result.answer, 'Local fallback for Keep going?');
  assert.equal(fallbackCalls, 1);
});
      ...
      async runTextTask({ task, input }) {
        ...
        if (task === 'source_digest' && Array.isArray(input?.sources)) {
          ...
          return {
            ...
            keyPoints: [{ text: 'Gateway consolidated digest.', evidence: chunk.text, chunkIds: [chunk.id] }],
            importantTerms: [],
            evidence: [{ claim: chunk.text, chunkIds: [chunk.id] }],
            conflicts: [],
            openQuestions: []
          };
        }
