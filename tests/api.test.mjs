  ...
  await withServer(async base => {
    ...
    assert.deepEqual(await response.json(), {
      ...
      voice: {
        ...
        transcriptMaxCharacters: 13_200,
        textTimeoutMs: 45_000,
        sourceDigestTimeoutMs: 180_000,
        sourceDigestMaxOutputTokens: 12_000,
        sourceConversationMaxOutputTokens: 3_300,
...
test('source question endpoint distinguishes source evidence from general context', async () => {
  await withServer(async base => {
    ...
    const sourceBody = await source.json();
    assert.equal(sourceBody.source.status, 'ready');
    assert.equal(sourceBody.source.digestStatus, 'ready');
    ...
    assert.equal(sourcesBody.digestStatus, 'ready');
    assert.equal(sourcesBody.sources[0].status, 'ready');
    assert.equal(sourcesBody.sources[0].digestStatus, 'ready');
...
test('source questions use the direct source digest before a consolidated digest is requested', async () => {
  await withServer(async base => {
    const create = await fetch(`${base}/api/sessions`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
...
test('voice source questions use the direct source digest before a consolidated digest is requested', async () => {
  const coach = {
    initialQuestion: async () => 'What is the main claim?',
    composeBlendedAnswer: async ({ citationChunks }) => ({
      answerText: 'Retrieval practice improves learning outcomes.',
      answerSpeechText: 'Retrieval practice improves learning outcomes.',
      sourceClaims: [{
        claim: 'Retrieval practice improves learning outcomes.',
        chunkId: citationChunks[0].id,
        citationExcerpt: 'Retrieval practice improves learning outcomes.'
      }],
      llmBackground: [],
      discussionPoints: [],
      suggestions: [],
      externalClaims: [],
      citations: [{
        sourceId: citationChunks[0].sourceId,
        chunkId: citationChunks[0].id,
        excerpt: 'Retrieval practice improves learning outcomes.'
      }],
      externalCitations: [],
      confidence: 'high',
      uncertainty: [],
      conflicts: [],
      followUp: 'What result should we examine next?'
    })
  };
  const server = createServer({ coach });
  await new Promise(resolve => server.listen(0, '127.0.0.1', resolve));
  try {
    ...
    const answer = await fetch(`${base}/api/voice/sessions/${created.session.id}/turns`, {
      ...
      headers,
      body: JSON.stringify({
        idempotencyKey: 'voice-direct-digest-1',
...
test('session summary reports the supplied material coverage', async () => {
  await withServer(async base => {
    ...
    assert.deepEqual(body.summary.learnedConcepts, ['This paper explains a useful study method.']);
...
test('source digest endpoints expose direct and consolidated digests with stored chunks', async () => {
  await withServer(async base => {
    const create = await fetch(`${base}/api/sessions`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
    ...
    const direct = await fetch(`${base}/api/sessions/${created.session.id}/sources/digest`, {
      headers: { 'x-session-token': created.token }
    });
    assert.equal(direct.status, 200);
    const directBody = await direct.json();
    assert.equal(directBody.status, 'ready');
    assert.equal(directBody.digest.keyPoints.length, 1);
...
test('ordinary source digest reuse does not resend completed source text for consolidation', async () => {
  let consolidatedCalls = 0;
  const coach = {
    initialQuestion: async () => 'What is the main idea?',
    digestSource: async source => ({
      mode: 'model',
      digestText: `Digest: ${source.text}`,
      keyPoints: [{ text: 'A supported point.', evidence: source.text }],
      openQuestions: []
    }),
    buildConsolidatedDigest: async ({ chunks }) => {
      consolidatedCalls += 1;
      return {
        mainArgument: chunks[0].text,
        keyPoints: [{ text: chunks[0].text, evidence: chunks[0].text, chunkIds: [chunks[0].id] }],
        importantTerms: [],
  ...
  try {
    ...
    const created = await (await fetch(`${base}/api/sessions`, {
      method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ topic: 'Reuse a digest', sourceMode: 'source' })
    })).json();
    const headers = { 'content-type': 'application/json', 'x-session-token': created.token };
    await fetch(`${base}/api/sessions/${created.session.id}/sources`, {
      method: 'POST', headers, body: JSON.stringify({ name: 'paper.txt', text: 'The source reports a clear result.' })
    });

    const reused = await fetch(`${base}/api/sessions/${created.session.id}/sources/digest`, {
      method: 'POST', headers, body: JSON.stringify({})
    });
    assert.equal(reused.status, 200);
    assert.equal(consolidatedCalls, 0);

    const forced = await fetch(`${base}/api/sessions/${created.session.id}/sources/digest`, {
      method: 'POST', headers, body: JSON.stringify({ forceModelConsolidation: true })
    });
...
test('turn and source-question endpoints reject oversized text', async () => {
  await withServer(async base => {
    ...
    const answer = await fetch(`${base}/api/sessions/${created.session.id}/turns`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-session-token': created.token },
      body: JSON.stringify({ answer: 'x'.repeat(13_201) })
    });
    const question = await fetch(`${base}/api/sessions/${created.session.id}/questions`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-session-token': created.token },
      body: JSON.stringify({ mode: 'general', question: 'x'.repeat(2_201) })
    });
...
test('source limits allow ten files and reject an eleventh with measured and configured values', async () => {
  ...
  try {
    ...
    for (let index = 0; index < 10; index += 1) {
      ...
      assert.equal(body.source.digestStatus, 'ready');
    }
...
test('session creation accepts retentionMode and exposes privacy-safe session settings', async () => {
  ...
  try {
    ...
    assert.equal(body.session.modelTokenBudget, 132_000);
