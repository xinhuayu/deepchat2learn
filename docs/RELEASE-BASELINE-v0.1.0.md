...
- Every live prompt carries the topic. Practice begins with three bounded discovery rounds; after refinement, it presents a scope-confirmation prompt and carries the persisted scope digest/gist with up to five compact recent exchanges. Source-answer conversation uses the prepared digest/gist, bounded evidence context, and three recent exchanges; generated source questions use the prepared digest and recent history rather than repeatedly sending the original material.
...
- Source preparation retains the complete extracted material locally, bounds direct provider digestion to 88,000 characters per source, and supports bounded chunk batching for explicit consolidation. Source-answer prompts use the prepared digest, compact evidence options, and three recent exchanges; generated source questions use the digest and recent history.
