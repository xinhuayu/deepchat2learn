...
export function createCoach() {
  return {
    ...
    sourceQuestion({ sources: suppliedSources = [], sourceDigest = null, conversationHistory, skillProfile = null }) {
      const sources = Array.isArray(suppliedSources) && suppliedSources.length
        ? suppliedSources
      ...
      if (skillProfile?.id === 'epi-research') {
        if (sourceCount > 1) return 'How do these studies differ in design, target population, and main estimand, and which assumptions matter most for comparing them?';
      ...
      if (skillProfile?.id === 'academic-conversation') {
        if (sourceCount > 1) return 'How do these materials agree or differ, and what evidence would help us explain the difference?';
        return `What is the central idea in “${sources[0]?.name || 'this material'}”, and which evidence would you like me to explain first?`;
      }
      if (sourceCount > 1) return 'How do these materials agree or differ, and what evidence supports your comparison?';
