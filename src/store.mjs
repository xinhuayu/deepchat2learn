...
export function countCompletedTurns(session) {
  ...
  const answeredVoiceTurns = Array.isArray(session?.voiceTurns)
    ? session.voiceTurns.filter(turn => turn?.status === 'answered' && !['control', 'new_question', 'end_session'].includes(turn?.intent)).length
    : 0;
...
function practiceFeedbackEntries(session) {
  const typedFeedback = (Array.isArray(session?.turns) ? session.turns : [])
    ...
    .filter(Boolean);
  const persistedVoiceTurns = (Array.isArray(session?.voiceTurns) ? session.voiceTurns : [])
    .filter(turn => turn?.status === 'answered' && !['control', 'new_question', 'end_session'].includes(turn?.intent));
  const persistedVoiceFeedback = persistedVoiceTurns
    .map(turn => turn?.feedback)
    .filter(Boolean);
  const persistedVoiceIds = new Set(persistedVoiceTurns.map(turn => turn?.id).filter(Boolean));
  const replayOnlyFeedback = voiceReplayResults(session)
    .filter(result => result?.countsAsAnswer !== false && result?.feedback && !persistedVoiceIds.has(result?.turn?.id))
    .map(result => result.feedback);
  return [...typedFeedback, ...persistedVoiceFeedback, ...replayOnlyFeedback];
}

const MAX_SESSION_REVIEW_RECORDS = 100;
...
function reviewSourceClaims(turn, session) {
  return (Array.isArray(turn?.citations) ? turn.citations : []).map(citation => {
    const source = (Array.isArray(session?.sources) ? session.sources : []).find(item => item?.id === citation?.sourceId);
    const excerpt = String(citation?.excerpt || '').trim();
    return {
      claim: excerpt,
      sourceId: citation?.sourceId || null,
      sourceName: source?.name || citation?.sourceName || 'Supplied material',
      page: citation?.page ?? null,
      section: citation?.section ?? null,
      evidence: excerpt,
      locator: Number.isInteger(citation?.start) && Number.isInteger(citation?.end)
        ? { type: 'character', start: citation.start, end: citation.end }
        : null,
      relevanceScore: null
    };
...
function reviewQuestionForVoiceTurn(turn) {
  if (['source_question', 'general_question'].includes(turn?.intent)) return String(turn?.transcript || '').trim();
  return String(turn?.question || turn?.transcript || '').trim();
}

function buildSessionReview(session) {
  const coachingRecords = (Array.isArray(session?.turns) ? session.turns : []).map((turn, position) => ({
  ...
  const voiceTurns = Array.isArray(session?.voiceTurns) ? session.voiceTurns : [];
  for (const [index, turn] of voiceTurns.entries()) {
    if (turn?.status !== 'answered' || ['control', 'new_question', 'end_session'].includes(turn?.intent)) continue;
    if (!turn?.feedback) continue;
    coachingRecords.push({
      createdAt: turn.createdAt,
      position: (Array.isArray(session?.turns) ? session.turns.length : 0) + index,
      value: {
        question: reviewQuestionForVoiceTurn(turn),
        answer: String(turn.transcript || '').trim(),
  ...
  const materialRecords = voiceTurns.map((turn, position) => {
    if (turn?.status !== 'answered' || ['control', 'new_question', 'end_session'].includes(turn?.intent) || turn?.feedback) return null;
    const sourceGroundedClaims = reviewSourceClaims(turn, session);
    return {
      createdAt: turn.createdAt,
      position,
      value: {
        question: reviewQuestionForVoiceTurn(turn),
        voice: turn.inputMode === 'voice',
        mode: ['source_question', 'source_answer'].includes(turn.intent) ? 'source' : 'general',
        createdAt: turn.createdAt || null,
        answer: {
          mode: ['source_question', 'source_answer'].includes(turn.intent) ? 'source' : 'general',
          answer: String(turn.answerText || '').trim(),
          sourceGroundedClaims,
          additionalContext: turn.knowledgeLayers?.includes('llm') && !sourceGroundedClaims.length
            ? [{ claim: 'This response uses general academic context.', label: 'Additional context' }]
            : [],
          unsupportedOrUnresolved: Array.isArray(turn.unsupportedOrUnresolved) ? turn.unsupportedOrUnresolved : [],
  ...
  return {
    transcript: chronologicalRecords(coachingRecords).slice(-MAX_SESSION_REVIEW_RECORDS).map(record => record.value),
    materialHistory: chronologicalRecords(materialRecords).slice(-MAX_SESSION_REVIEW_RECORDS).map(record => record.value)
  };
}
...
export class InMemoryStore {
  ...
  publicSession(session) {
    ...
    return {
      ...
      review: buildSessionReview(session)
    };
  }
