  ...
  constructor({
    ...
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS sessions (
        ...
        model_token_budget INTEGER NOT NULL DEFAULT 120000,
  ...
  save(session) {
    ...
    try {
      ...
      for (const turn of session.voiceTurns) this.db.prepare(`INSERT INTO voice_turns (
        id, session_id, sequence, input_mode, transcript, transcript_confidence, transcript_reviewed, intent, status,
        answer_text, answer_speech_text, knowledge_layers_json, citations_json, external_citations_json, discussion_points_json, suggestions_json, confidence,
...
function ensureSessionCollections(session) {
  ...
  if (!('modelTokenBudget' in session)) session.modelTokenBudget = 120_000;
