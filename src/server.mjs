...
import { buildConsolidatedDigest, chunkSource } from './sourceKnowledge.mjs';
...
import './config.mjs';
import { getRequestConfig, getResearchConfig, getRetentionConfig, getSessionBudgetConfig, getSourceLimits, getVoiceConfig } from './config.mjs';
  ...
  ? createResilientCoach(
    ...
    createCoach(),
    {
      onFallback: ({ method, error }) => {
        console.warn(`[deepchat2learn] text model fallback for ${method}: ${error?.code || 'MODEL_ERROR'} ${error?.message || 'request failed'}`);
      }
    }
  )
  : createCoach();
...
const configuredOrchestrator = createConversationOrchestrator({
  ...
  config: {
    maxAnswerCharacters: Number(process.env.MAX_ANSWER_CHARACTERS) || 12_000,
    maxQuestionCharacters: Number(process.env.MAX_QUESTION_CHARACTERS) || 2_000,
    maxVoiceTranscriptCharacters: configuredVoice.transcriptMaxCharacters
  }
});
const maxAnswerCharacters = Number(process.env.MAX_ANSWER_CHARACTERS) || 12_000;
const maxQuestionCharacters = Number(process.env.MAX_QUESTION_CHARACTERS) || 2_000;
...
async function handleApi(req, res, url, runtime = {}) {
  ...
  if (method === 'POST' && parts.length === 4 && parts[3] === 'source-prompts') {
    ...
    consumeModelBudget(session, session.topic, session.sources.map(source => source.text), session.sourceDigest);
    const question = await directTextCoach.sourceQuestion({ topic: session.topic, sources: session.sources, sourceDigest: session.sourceDigest || null, skillProfile: skillRegistry.get('academic-conversation') });
