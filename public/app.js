...
function applyVoiceEventsState(event) {
  ...
  if (event.type === 'permission_pending') {
    ...
    setVoiceAnnouncement(event.questionPrompt
      ...
      : (voiceCoordinator.closingMessage || 'AI is speaking. You can interrupt at any time.'));
    setVoiceConversationState('speaking');
    ...
    if (voiceCoordinator.transport === 'browser-fallback' && voiceCoordinator.active && voiceCoordinator.shouldAutoResume) {
      ...
      const closingMessage = voiceCoordinator.closingMessage;
      voiceCoordinator.active = false;
      voiceCoordinator.state = 'idle';
      transitionBrowserConversationState('completed', {
        legacyMode: 'off',
        announcement: closingMessage || 'This session reached its question limit. Wrapping up your summary...'
      });
      clearVoiceCompletionTimer();
      voiceCoordinator.completionTimer = window.setTimeout(() => {
        voiceCoordinator.completionTimer = null;
        if (state.session?.status === 'ready_to_complete') completeSession({ closingMessage }).catch(notifyError);
...
function renderBrowserConversationState() {
  ...
  const voiceCaption = $('#voiceCaptionText');
  ...
  const statusHighlighted = ['ai-speaking', 'processing'].includes(browserState);
  ...
  if (voiceState) {
    ...
    voiceState.classList.toggle('voice-processing-message', statusHighlighted);
  }
  ...
  if (voiceCaption) voiceCaption.classList.remove('voice-processing-message');
...
const voiceCoordinator = {
  ...
  start: async ({ transport = 'browser-fallback', reconnecting = false } = {}) => {
    ...
    voiceCoordinator.closingMessage = '';
    clearVoiceSilenceTimer();
  ...
  submitTranscript: async ({ transcript, confidence, reviewed, itemKey } = {}) => {
    ...
    voiceCoordinator.closingMessage = result?.sessionEnded ? (result.closingMessage || 'Thanks for the conversation. Your session is complete; here is your summary.') : '';
    state.voiceReviewPending = false;
    if (result.sessionEnded) {
      persistClientSession();
      emitAndApplyVoiceEvent('answer_approved', { answerSpeechText: result.answerSpeechText, result });
      voiceCoordinator.externalResearchSpeechText = '';
    ...
    if (result?.feedback) {
      ...
      if (!state.transcript.some(turn => turn.question === questionBeforeTurn && turn.answer === trimmed)) {
        state.transcript.push({ question: questionBeforeTurn, answer: trimmed, feedback: result.feedback, voice: true, createdAt: new Date().toISOString(), reviewSequence: nextReviewSequence() });
        renderTranscript();
      }
    } else {
      const answer = renderApprovedAnswer(result, { speakAnswer: false });
      if (result.countsAsAnswer !== false) {
        state.materialHistory.push({ question: trimmed, answer, createdAt: new Date().toISOString(), reviewSequence: nextReviewSequence() });
        renderTranscript();
      }
    }
...
function nextReviewSequence() {
  state.reviewSequence = Number(state.reviewSequence || 0) + 1;
  return state.reviewSequence;
}
...
function renderTranscript() {
  ...
  const reviewMarkup = orderedReviewEntries().map(entry => {
    ...
    const label = item.mode === 'source' ? 'Materials Q&amp;A' : 'Question &amp; answer';
    return `<article class="transcript-turn material-review"><div class="transcript-label">${label} ${state.materialHistory.length - entry.index}</div><p class="transcript-question">${escapeHtml(item.question)}</p><div class="transcript-label">Answer</div><p>${escapeHtml(answer.answer || '')}</p>${(answer.sourceGroundedClaims || []).map(claim => `<details class="transcript-evidence"><summary>Supporting evidence from ${escapeHtml(claim.sourceName)}</summary><p>${escapeHtml(claim.evidence)}</p></details>`).join('')}</article>`;
  });
  list.innerHTML = reviewMarkup.join('');
...
function transcriptText() {
  ...
  const turnsText = state.transcript.map((turn, index) => {
    const feedback = turn.feedback;
    return [
      `Question ${index + 1}: ${turn.question}`,
      `Your answer: ${turn.answer}`,
      ...(feedback ? ['Strengths:', ...(feedback.strengths || []).map(item => `- ${item}`), `Try next: ${feedback.improvement || 'Review one specific improvement.'}`, `Why this feedback: ${feedback.evidence?.join(' ') || 'Based on the answer you submitted.'}`] : [])
    ].join('\n');
  }).join('\n\n');
...
async function submitAnswer({ voice = false, answer: submittedAnswer } = {}) {
  ...
  try {
    ...
    if (result.sessionEnded) {
      state.voiceReviewPending = false;
      state.session.status = 'ready_to_complete';
      const closingMessage = result.closingMessage || 'Thanks for the conversation. Your session is complete; here is your summary.';
      if (voice && state.voiceConversation === 'submitting') {
        setVoiceConversationState('speaking');
        speak(closingMessage, { onend: () => completeSession({ closingMessage }).catch(notifyError), onerror: () => completeSession({ closingMessage }).catch(notifyError) });
      } else {
        await completeSession({ closingMessage });
      }
      return;
    }
    ...
    if (!state.transcript.some(turn => turn.question === question && turn.answer === answer)) {
      state.transcript.push({ question, answer, feedback: result.feedback, createdAt: new Date().toISOString(), reviewSequence: nextReviewSequence() });
      renderTranscript();
    }
    state.voiceReviewPending = false;
...
async function askMaterialQuestion() {
  ...
  try {
    ...
    if (answer?.sessionEnded) {
      await completeSession({ closingMessage: answer.closingMessage || 'Thanks for the conversation. Your session is complete; here is your summary.' });
      return;
    }
    const approved = renderApprovedAnswer(answer);
    state.materialHistory.push({ question, answer: approved, createdAt: new Date().toISOString(), reviewSequence: nextReviewSequence() });
    persistClientSession();
  } catch (error) {
...
async function completeSession({ closingMessage = '' } = {}) {
  clearVoiceCompletionTimer();
  ...
  $('#summaryStatus').textContent = closingMessage;
  show('summaryView');
  $('#summaryView').focus();
  clearClientSession();
  return;
...
async function restoreClientSession() {
  ...
  try {
    ...
    state.transcript = Array.isArray(review.transcript)
      ? review.transcript
      : (Array.isArray(saved.transcript) ? saved.transcript.slice(0, result.session.turnCount) : []);
    state.materialHistory = Array.isArray(review.materialHistory)
      ? review.materialHistory
      : (Array.isArray(saved.materialHistory) ? saved.materialHistory.slice(-20) : []);
    state.reviewSequence = state.transcript.length + state.materialHistory.length;
